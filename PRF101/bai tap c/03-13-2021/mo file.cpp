...
void printmenu(int &c)
{
	...
	printf("=		1) Print  list                             =\n");
	printf("=		2) Rewrite list.                           =\n");
	printf("=		3) Edit an employer.                       =\n");
	printf("=		4) Add an employer.                        =\n");
	printf("=		5) Delete an employer.                     =\n");
...
void xuat(employ a[],int n)
{
	...
	//================================
	fprintf(fpt, "\n\nHighest salary: %d", a[n].EmID);
	//================================
	fprintf(fpt,"\nTotal of employers: %d",n);
	//===============================
	fprintf(fpt,"\nSame name employers:");
	int c=0,d=0;
	char b[50];
	for (int i=1; i<=n; i++)
	{
		...
		for (int j=i+1; j<=n; j++)
		{
			if (strcmp(b,a[j].EmName)==0)
			{
				fprintf(fpt," %d",a[i].EmID);
				fprintf(fpt," %d",a[j].EmID);
				fprintf(fpt,";");
				c++;
			}
		}
...
int main() 
{
	FILE *data;
	int n,i,c;
	int sd=demSoDong("D:\\data.txt");
	n=sd/3;
	employ a[100];
	//lay du lieu ra
	data=fopen("D:\\data.txt","r");
	for (int i=1; i<=sd; i++)
	{
		fscanf(data,"%d\n",&a[i].EmID);	
		fscanf(data,"%s\n",&a[i].EmName);
